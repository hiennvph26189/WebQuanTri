...
import {getAllNews,addNewsService,editNewsService} from "../../../services/newService"
...
class ModalEditNews extends Component {
    ...
    componentDidMount() {
        let itemNew = this.props.currentNews
        if(itemNew && !_.isEmpty(itemNew)){
            this.setState({
                tieuDe: itemNew.tieuDe,
                anhTieuDe: itemNew.anhTinTuc,
                tomTatTinTuc: itemNew.tomTatTinTuc,
                mota: itemNew.moTa,
                id: itemNew.id
    ...
    handleEditNews = async() => {
        ...
        if(isValid === true) {
           ...
           let data =  await editNewsService({
        ...
        <Modal 
         ...
         >
            <ModalHeader toggle={()=>this.toggle()}>Create New Category</ModalHeader>
                ...
                <ModalFooter>
                <Button color="success" className='px-2' onClick={()=>this.handleEditNews()}>
                    Edit News
                </Button>{' '}
